...
GIORNI = 3
FILE_COSTA = Path("comuni_costieri_calabria.csv")
...
CODICI = {
    ...
    53: ("🌦️", "Pioviggine moderata"), 55: ("🌧️", "Pioviggine intensa"), 56: ("🌧️", "Pioggia gelata debole"),
    57: ("🌧️", "Pioggia gelata intensa"), 61: ("🌧️", "Pioggia debole"), 63: ("🌧️", "Pioggia moderata"),
    ...
    77: ("❄️", "Granelli di neve"), 80: ("🌦️", "Rovesci deboli"), 81: ("🌧️", "Rovesci moderati"),
    82: ("🌧️", "Rovesci forti"), 85: ("🌨️", "Rovesci nevosi deboli"), 86: ("🌨️", "Rovesci nevosi forti"),
...
MARINE_CURRENT = ["wave_height", "wave_direction", "wave_period", "wave_peak_period", "wind_wave_height", "wind_wave_direction", "wind_wave_period", "swell_wave_height", "swell_wave_direction", "swell_wave_period", "sea_surface_temperature"]
MARINE_DAILY = ["wave_height_max", "wave_direction_dominant", "wave_period_max", "wind_wave_height_max", "swell_wave_height_max", "swell_wave_direction_dominant"]
...
def direzione(x):
    try:
        if x is None or pd.isna(x):
            return "—"
        return ["N", "NE", "E", "SE", "S", "SO", "O", "NO"][int((float(x) + 22.5) // 45) % 8]
    except (TypeError, ValueError):
        return "—"
...
# -----------------------------------------------------------------------------
# ICONA SVG NOTTURNA: luna e nube sono elementi sovrapposti nello stesso SVG
# -----------------------------------------------------------------------------
...
def carica_comuni_costieri():
    if not FILE_COMUNI_COSTIERI.exists():
        raise FileNotFoundError("Manca comuni_costieri_calabria.csv nella cartella di app.py.")
    dataframe = pd.read_csv(FILE_COMUNI_COSTIERI)
    if "comune" not in dataframe.columns:
        raise ValueError("Il CSV deve avere una colonna 'comune'.")
    return {normalizza_comune(x) for x in dataframe["comune"].dropna().astype(str) if x.strip()}


@st.cache_data(ttl=86400, show_spinner=False)
def geocodifica_calabria(nome):
    try:
        geocoder = Nominatim(user_agent="calabria_meteo_lab_final", timeout=15)
        risposta = geocoder.geocode(f"{nome}, Calabria, Italia", exactly_one=True, addressdetails=True, timeout=15)
    ...
    if not ("calabria" in regione or (37.75 <= lat <= 40.15 and 15.60 <= lon <= 17.25)):
        raise ValueError(f"❌ «{nome}» non è in Calabria.")

    risolto = indirizzo.get("city") or indirizzo.get("town") or indirizzo.get("village") or indirizzo.get("hamlet") or indirizzo.get("municipality") or nome.title()
    chiavi = ["municipality", "city", "town", "village", "hamlet", "suburb", "city_district", "district", "borough", "county"]
    candidati = list(dict.fromkeys([nome, risolto] + [str(indirizzo[k]) for k in chiavi if indirizzo.get(k)]))

    return risolto, lat, lon, candidati
...
def risolvi_localita(testo):
    ...
    indice = {nome.casefold(): nome for nome in COMUNI_RAPIDI}
    nome_norm = ALIASES.get(nome.casefold(), nome)

    if nome_norm.casefold() in indice:
        comune = indice[nome_norm.casefold()]
        lat, lon = COMUNI_RAPIDI[comune]
        return comune, lat, lon, [comune, nome_norm, nome]
...
def scarica_previsione_terrestre(lat, lon):
    params = {
        "latitude": lat, "longitude": lon, "models": MODELLO,
        "timezone": "Europe/Rome", "forecast_days": GIORNI_PREVISIONE,
        "temperature_unit": "celsius", "wind_speed_unit": "kmh",
        "precipitation_unit": "mm", "current": ",".join(VARIABILI_CORRENTI),
        "hourly": ",".join(VARIABILI_ORARIE), "daily": ",".join(VARIABILI_GIORNALIERE),
    }
    try:
        risposta = requests.get(API_METEO_URL, params=params, timeout=25)
...
def scarica_previsione_mare(lat, lon):
    params = {
        ...
        "forecast_days": GIORNI_PREVISIONE, "cell_selection": "sea",
        "current": ",".join(VARIABILI_MARINE_CORRENTI),
        "hourly": ",".join(VARIABILI_MARINE_ORARIE),
        "daily": ",".join(VARIABILI_MARINE_GIORNALIERE),
    }
    try:
        risposta = requests.get(API_MARE_URL, params=params, timeout=25)
        risposta.raise_for_status()
        dati = risposta.json()
    except requests.RequestException as exc:
        raise RuntimeError(f"Errore nella ricezione della previsione marina: {exc}") from exc

    distanza = distanza_haversine_km(lat, lon, float(dati.get("latitude", lat)), float(dati.get("longitude", lon)))
...
def calcola_dati_diurni(ore_giorno, alba, tramonto):
    ...
    if alba is not None and tramonto is not None:
        target = ore_giorno.loc[(ore_giorno["time"] >= pd.Timestamp(alba)) & (ore_giorno["time"] <= pd.Timestamp(tramonto))]
    else:
        target = ore_giorno.loc[(ore_giorno["time"].dt.hour >= 7) & (ore_giorno["time"].dt.hour <= 20)]
...
def prepara_dati_terrestri(dati):
    ...
    for _, riga in ore.iterrows():
        ...
        if righe.empty:
            ...
            g = righe.iloc[0]
            notti.append(e_notte(riga["time"], g.get("sunrise"), g.get("sunset")))
...
def stato_mare_da_onda(h):
    try:
        if h is None or pd.isna(h):
            return "Non disponibile", "⚪", "cml-mare-nd"
        h = float(h)
        if h < .10: return "Calmo", "🟦", "cml-mare-calmo"
        if h < .50: return "Quasi calmo", "🟦", "cml-mare-quasi-calmo"
        if h < 1.25: return "Poco mosso", "🟩", "cml-mare-poco-mosso"
        if h < 2.50: return "Mosso", "🟨", "cml-mare-mosso"
        if h < 4.00: return "Molto mosso", "🟧", "cml-mare-molto-mosso"
        if h < 6.00: return "Agitato", "🟥", "cml-mare-agitato"
...
def valuta_rischio_locale(riga):
    p = float(riga.get("precipitation_sum", 0) or 0)
    g = float(riga.get("wind_gusts_10m_max", 0) or 0)
    c = int(riga.get("weather_code_prevalente", riga.get("weather_code", 0)))
    if p >= 100 or g >= 100 or c in [96, 99]: return "rosso", "temporali" if c in [95, 96, 99] else ("vento" if g >= 100 else "precipitazioni")
    if p >= 50 or g >= 70 or c in [95, 96, 99]: return "arancione", "temporali" if c in [95, 96, 99] else ("vento" if g >= 70 else "precipitazioni")
    if p >= 20 or g >= 50 or c in [80, 81, 82]: return "giallo", "rovesci" if c in [80, 81, 82] else ("vento" if g >= 50 else "precipitazioni")
    return "verde", "nessuna criticità"


def badge_rischio_html(livello, rischio):
    palette = {"verde": ("#12855c", "✅", "Nessuna criticità stimata"), "giallo": ("#b77906", "⚠️", "Attenzione meteorologica"), "arancione": ("#d85d05", "🟠", "Rischio meteorologico elevato"), "rosso": ("#be2635", "🔴", "Rischio meteorologico molto elevato")}
...
def genera_app_completa(luogo, lat, lon, dati, ore, giorni, dati_mare=None, distanza_mare_km=None):
    cur = dati["current"]
    ...
    metriche_html = "".join(f'<div class="cml-metric-card"><div class="cml-metric-icon">{i}</div><div><div class="cml-metric-label">{html.escape(e)}</div><div class="cml-metric-value">{html.escape(v)}</div></div></div>' for i, e, v in metriche)

    mare_html = ""
    if dati_mare is not None and dati_mare.get("current"):
        cm = dati_mare["current"]
        stato, ico_mare, classe = stato_mare_da_onda(cm.get("wave_height"))
        mare_html = f'''<section class="cml-marine-box"><div class="cml-marine-head"><div><span class="cml-eyebrow">BOLLETTINO COSTIERO</span><h2>🌊 Vento e stato del mare</h2><p>Previsione marina per il comune costiero. Cella a circa {numero(distanza_mare_km, 1, " km")} dal punto selezionato.</p></div><div class="cml-sea-status {classe}"><span>{ico_mare}</span><div><small>STATO DEL MARE</small><strong>{html.escape(stato)}</strong></div></div></div><div class="cml-marine-grid"><div class="cml-marine-card"><span>🌊 Altezza onda</span><strong>{numero(cm.get("wave_height"), 2, " m")}</strong><small>Onda significativa</small></div><div class="cml-marine-card"><span>🧭 Provenienza onda</span><strong>{direzione(cm.get("wave_direction"))}</strong><small>{numero(cm.get("wave_direction"), 0, "°")}</small></div><div class="cml-marine-card"><span>〰️ Periodo medio</span><strong>{numero(cm.get("wave_period"), 1, " s")}</strong><small>Intervallo medio</small></div><div class="cml-marine-card"><span>📈 Periodo picco</span><strong>{numero(cm.get("wave_peak_period"), 1, " s")}</strong><small>Energia dominante</small></div><div class="cml-marine-card"><span>💨 Mare del vento</span><strong>{numero(cm.get("wind_wave_height"), 2, " m")}</strong><small>Wind sea</small></div><div class="cml-marine-card"><span>🌐 Mare di fondo</span><strong>{numero(cm.get("swell_wave_height"), 2, " m")}</strong><small>Swell</small></div><div class="cml-marine-card"><span>🌡️ Temperatura mare</span><strong>{numero(cm.get("sea_surface_temperature"), 1, " °C")}</strong><small>SST</small></div></div><div class="cml-marine-note">ℹ️ La direzione indica da dove proviene il moto ondoso. I valori sono modellistici e non sostituiscono fonti nautiche o avvisi ufficiali.</div></section>'''

    carte = []
    etichette = ["OGGI", "DOMANI", "DOPODOMANI"]
    for idx, (_, r) in enumerate(giorni.iterrows()):
        codice = r.get("weather_code_prevalente", r.get("weather_code"))
        ico, desc = meteo(codice)
        livello, rischio = valuta_rischio_locale(r)
        classe = "cml-icon-thunder" if codice in [95, 96, 99] else "cml-icon-snow" if codice in range(71, 87) else "cml-icon-rain" if codice in range(51, 68) or codice in range(80, 83) else "cml-icon-cloud" if codice in [2, 3, 45, 48] else "cml-icon-sun"
        carte.append(f'<article class="cml-day-card"><div class="cml-day-head"><span class="cml-day-tag">{etichette[idx] if idx < len(etichette) else "PROSSIMAMENTE"}</span><span class="cml-day-date">{html.escape(data_it(r["time"]))}</span></div><div class="cml-risk-row">{badge_rischio_html(livello, rischio)}</div><div class="cml-day-weather"><div class="cml-day-icon {classe}">{ico}</div><div><div class="cml-day-description">{html.escape(desc)}</div><div class="cml-day-moon">{html.escape(str(r.get("Fase lunare", "🌙 Luna")))}</div></div></div><div class="cml-temperature-grid"><div class="cml-temp-box"><span>MINIMA</span><strong class="cml-temp-min">↓ {numero(r["temperature_2m_min"], 1, "°")}</strong></div><div class="cml-temp-box"><span>MASSIMA</span><strong class="cml-temp-max">↑ {numero(r["temperature_2m_max"], 1, "°")}</strong></div></div><div class="cml-day-details"><div><span>☁️ Nuvolosità</span><b>{numero(r.get("cloud_cover_diurno"), 0, " %")}</b></div><div><span>🌧️ Precipitazione</span><b>{numero(r.get("precipitation_sum"), 1, " mm")}</b></div><div><span>💨 Vento massimo</span><b>{numero(r.get("wind_speed_10m_max"), 0, " km/h")}</b></div><div><span>🌬️ Raffica massima</span><b>{numero(r.get("wind_gusts_10m_max"), 0, " km/h")}</b></div><div><span>🧭 Direzione</span><b>{html.escape(str(r.get("Da", "—")))}</b></div></div><div class="cml-astro-grid"><div><span>☀️ Alba</span><b>{ora_it(r.get("sunrise"))}</b></div><div><span>🌇 Tramonto</span><b>{ora_it(r.get("sunset"))}</b></div><div><span>🌙 Sorge</span><b>{ora_it(r.get("moonrise"))}</b></div><div><span>🌘 Tramonta</span><b>{ora_it(r.get("moonset"))}</b></div></div></article>')

    date_disponibili = sorted(ore["time"].dt.date.unique())
    if not date_disponibili:
        raise RuntimeError("Nessun dato orario disponibile.")

    dati_grafici, pulsanti, tabelle = {}, [], []
    for idx, giorno in enumerate(date_disponibili):
        chiave = str(giorno)
        og = ore.loc[ore["time"].dt.date == giorno]
        dati_grafici[chiave] = {"ore": og["time"].dt.strftime("%H:%M").tolist(), "temperatura": [round(float(v), 1) if pd.notna(v) else None for v in og["temperature_2m"]], "vento": [round(float(v), 1) if pd.notna(v) else None for v in og["wind_speed_10m"]], "precipitazione": [round(float(v), 1) if pd.notna(v) else None for v in og["precipitation"]]}
        pulsanti.append(f'<button class="cml-tab-btn {"active" if idx == 0 else ""}" type="button" onclick="mostraGiorno(\'{chiave}\',this)">📅 {html.escape(data_it(giorno).title())}</button>')
        righe = []
        for _, r in og.iterrows():
            notte = "cml-night-row" if bool(r.get("Notte", False)) else ""
            righe.append(f'<tr class="{notte}"><td>{r["time"].strftime("%H:%M")}</td><td class="cml-scenario-cell"><span class="cml-table-icon">{r["Icona"]}</span><span>{html.escape(str(r["Scenario"]))}</span></td><td>{numero(r["temperature_2m"],1)}</td><td>{numero(r["apparent_temperature"],1)}</td><td>{numero(r["precipitation"],1)}</td><td>{numero(r["wind_speed_10m"],0)}</td><td>{html.escape(str(r["Da"]))}</td><td>{numero(r["wind_gusts_10m"],0)}</td><td>{numero(r["cloud_cover"],0)}</td><td>{numero(r["relative_humidity_2m"],0)}</td></tr>')
        tabelle.append(f'<div id="tab-{chiave}" class="cml-day-table-container" style="{"display:block;" if idx == 0 else "display:none;"}"><div class="cml-table-wrap"><table class="cml-table"><thead><tr><th>Ora</th><th>Scenario</th><th>Temp. °C</th><th>Percepita °C</th><th>Pioggia mm</th><th>Vento km/h</th><th>Da</th><th>Raffica km/h</th><th>Nubi %</th><th>Umidità %</th></tr></thead><tbody>{"".join(righe)}</tbody></table></div></div>')

    iniziale = str(date_disponibili[0])
    grafici_json = json.dumps(dati_grafici, ensure_ascii=False)

    documento = f'''<!DOCTYPE html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"><script src="https://cdn.jsdelivr.net/npm/chart.js"></script><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script><style>
:root{{--ink:#102b3b;--muted:#607987;--orange:#f26e17;--page:#eef5f8}}*{{box-sizing:border-box}}body{{margin:0;padding:8px;min-height:100vh;color:var(--ink);background:var(--page);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Arial,sans-serif;line-height:1.45}}.cml-hero{{margin:8px 0 22px;padding:42px 46px 38px;border:1px solid #07516c;border-radius:28px;color:#fff;background:linear-gradient(135deg,#06324d,#075b78 52%,#087f91);box-shadow:0 14px 32px rgba(9,61,83,.22)}}.cml-brand{{display:inline-flex;align-items:center;gap:10px;padding:8px 15px;border:1px solid rgba(255,255,255,.25);border-radius:999px;background:rgba(0,35,55,.35);color:#d7f7fa;font-size:11px;font-weight:800;letter-spacing:2px}}.cml-brand-mark{{width:11px;height:11px;border-radius:50%;background:#ffd45c}}.cml-hero h1{{margin:19px 0 10px;color:#fff;font-size:44px;font-weight:800}}.cml-hero p{{max-width:760px;margin:0;color:#e2f5f8;font-size:15px;line-height:1.7}}.cml-current,.cml-marine-box,.cml-radar-box,.cml-day-card,.cml-chart-box{{border:1px solid #d5e4e9;border-radius:24px;background:#fff;box-shadow:0 8px 28px rgba(23,67,84,.10)}}.cml-current{{overflow:hidden;margin:22px 0 25px}}.cml-current-main{{display:flex;align-items:center;justify-content:space-between;gap:28px;padding:34px 38px;border-bottom:1px solid #e2edf0}}.cml-kicker{{display:inline-flex;align-items:center;gap:8px;padding:7px 13px;border-radius:999px;background:#e3f5f8;color:#087087;font-size:11px;font-weight:800;letter-spacing:1.2px}}.cml-live-dot{{width:8px;height:8px;border-radius:50%;background:#18a579}}.cml-place-block h2{{margin:13px 0 10px;font-size:35px}}.cml-condition{{display:inline-flex;align-items:center;gap:8px;padding:8px 13px;border:1px solid #e1edf0;border-radius:11px;color:#4d6570;background:#f8fbfc;font-size:16px}}.cml-temperature{{display:flex;align-items:flex-start;color:var(--orange);font-weight:900;line-height:.9;white-space:nowrap}}.cml-temperature span{{font-size:84px;letter-spacing:-7px}}.cml-temperature small{{margin:10px 0 0 8px;font-size:28px}}.cml-metrics{{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;padding:22px 28px}}.cml-metric-card{{display:flex;align-items:center;gap:13px;min-height:70px;padding:14px 16px;border:1px solid #dce9ed;border-radius:15px;background:#f7fbfc}}.cml-metric-icon{{display:flex;align-items:center;justify-content:center;width:42px;height:42px;border:1px solid #e2edef;border-radius:12px;background:#fff;font-size:23px}}.cml-metric-label{{color:var(--muted);font-size:10px;font-weight:800;letter-spacing:.7px;text-transform:uppercase}}.cml-metric-value{{margin-top:4px;font-size:17px;font-weight:850}}.cml-current-footer{{display:flex;flex-wrap:wrap;gap:8px 25px;padding:14px 28px;border-top:1px solid #e3edf0;background:#f8fbfc;color:var(--muted);font-size:12px}}.cml-marine-box{{margin:0 0 30px;padding:26px}}.cml-marine-head{{display:flex;align-items:center;justify-content:space-between;gap:18px;margin-bottom:20px;flex-wrap:wrap}}.cml-eyebrow{{color:#087087;font-size:10px;font-weight:900;letter-spacing:1.8px}}.cml-marine-head h2{{margin:6px 0 4px;font-size:25px}}.cml-marine-head p{{margin:0;color:var(--muted);font-size:12px}}.cml-sea-status{{display:flex;align-items:center;gap:10px;min-width:190px;padding:13px 16px;border-radius:14px;color:#fff}}.cml-sea-status>span{{font-size:28px}}.cml-sea-status small{{display:block;color:rgba(255,255,255,.84);font-size:10px;font-weight:800}}.cml-sea-status strong{{font-size:17px}}.cml-mare-calmo{{background:#0d77a7}}.cml-mare-quasi-calmo{{background:#168db7}}.cml-mare-poco-mosso{{background:#278b5e}}.cml-mare-mosso{{background:#c18a1c}}.cml-mare-molto-mosso{{background:#d16418}}.cml-mare-agitato{{background:#c3313d}}.cml-mare-molto-agitato{{background:#8b1e29}}.cml-mare-nd{{background:#6d7d86}}.cml-marine-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(165px,1fr));gap:12px}}.cml-marine-card{{display:flex;flex-direction:column;gap:5px;min-height:105px;padding:15px;border:1px solid #dbe8ec;border-radius:15px;background:#f7fbfc}}.cml-marine-card span{{color:#54707c;font-size:12px;font-weight:700}}.cml-marine-card strong{{color:#143b4b;font-size:22px}}.cml-marine-card small{{color:#768c95;font-size:11px}}.cml-marine-note{{margin-top:16px;padding:12px 15px;border:1px solid #bfe1ea;border-left:4px solid #168db7;border-radius:11px;background:#ecf9fc;color:#3d6270;font-size:12px;line-height:1.6}}.cml-radar-box{{margin-bottom:32px;padding:26px}}.cml-radar-head{{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:16px;flex-wrap:wrap}}.cml-radar-head h2{{margin:0;font-size:23px}}.cml-radar-head p{{margin:3px 0;color:var(--muted);font-size:12px}}.cml-radar-btn{{border:0;border-radius:11px;padding:10px 16px;background:linear-gradient(135deg,#0c7f96,#075d74);color:#fff;cursor:pointer;font-weight:800}}#radar-map{{width:100%;height:480px;border:1px solid #d8e7eb;border-radius:16px}}.cml-radar-footer{{display:flex;justify-content:space-between;gap:10px;margin-top:14px;padding:12px 15px;border:1px solid #dcebef;border-radius:12px;background:#f5fafb;color:var(--muted);font-size:12px}}.cml-radar-time{{color:#087087;font-weight:850}}.cml-section-title{{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;margin:38px 0 18px}}.cml-section-title span{{color:#0b7f98;font-size:10px;font-weight:900;letter-spacing:1.8px}}.cml-section-title h2{{margin:7px 0 5px;font-size:28px}}.cml-section-title p{{margin:0;color:var(--muted);font-size:13px}}.cml-pill{{padding:8px 14px;border:1px solid #bfe2e8;border-radius:999px;background:#e7f6f8;color:#087087;font-size:12px;font-weight:850}}.cml-days-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:18px}}.cml-day-card{{padding:23px}}.cml-day-head{{display:flex;justify-content:space-between;gap:10px}}.cml-day-tag{{padding:6px 12px;border-radius:999px;background:linear-gradient(135deg,#0e95ab,#087087);color:#fff;font-size:10px;font-weight:900}}.cml-day-date{{color:var(--muted);font-size:12px;font-weight:700}}.cml-risk-row{{margin:14px 0}}.cml-risk-badge{{display:inline-flex;gap:7px;padding:7px 10px;border-radius:10px;color:#fff;font-size:11px;font-weight:800}}.cml-day-weather{{display:flex;align-items:center;gap:16px;margin-bottom:20px}}.cml-day-icon{{display:flex;align-items:center;justify-content:center;width:68px;height:68px;border-radius:19px;font-size:35px}}.cml-icon-sun{{background:linear-gradient(135deg,#ffe99a,#f9b843)}}.cml-icon-cloud{{background:linear-gradient(135deg,#dbe6ea,#93aab5)}}.cml-icon-rain{{background:linear-gradient(135deg,#9ed4ef,#327eae)}}.cml-icon-snow{{background:linear-gradient(135deg,#eff9ff,#b8d5e3)}}.cml-icon-thunder{{background:linear-gradient(135deg,#cbb3e6,#67458b)}}.cml-day-description{{font-size:18px;font-weight:850}}.cml-day-moon{{display:inline-flex;margin-top:6px;padding:5px 9px;border-radius:999px;background:#eef2ff;color:#48538f;font-size:11px}}.cml-temperature-grid{{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:17px;padding:13px;border:1px solid #e1ebee;border-radius:15px;background:#f8fbfc}}.cml-temp-box{{display:flex;flex-direction:column;align-items:center;gap:4px}}.cml-temp-box span{{color:var(--muted);font-size:10px;font-weight:850}}.cml-temp-box strong{{font-size:24px}}.cml-temp-min{{color:#167aa9}}.cml-temp-max{{color:var(--orange)}}.cml-day-details{{display:grid;gap:9px;margin-bottom:18px;padding-bottom:17px;border-bottom:1px solid #e4edef}}.cml-day-details div{{display:flex;justify-content:space-between;color:#536d78;font-size:13px}}.cml-day-details b{{color:#234b59}}.cml-astro-grid{{display:grid;grid-template-columns:1fr 1fr;gap:9px}}.cml-astro-grid div{{display:flex;justify-content:space-between;padding:9px;border:1px solid #e3edef;border-radius:10px;background:#fafcfd;color:#637b85;font-size:11px}}.cml-note{{margin:18px 0 25px;padding:15px 18px;border:1px solid #f0dca8;border-left:5px solid #dc9c2d;border-radius:14px;background:#fff8e7;color:#67552d;font-size:13px;line-height:1.6}}.cml-hour-header{{margin-top:12px;padding:29px 31px;border-radius:22px;background:linear-gradient(135deg,#122a50,#274d85);color:#fff}}.cml-hour-header span{{color:#aeeaf2;font-size:10px;font-weight:850}}.cml-hour-header h2{{margin:9px 0 6px;font-size:28px}}.cml-hour-header p{{margin:0;color:#dceaff;font-size:13px}}.cml-chart-box{{margin:20px 0 14px;padding:20px}}.cml-chart-title{{font-size:18px;font-weight:850}}.cml-chart-subtitle{{margin-bottom:15px;color:var(--muted);font-size:12px}}.cml-chart-canvas-wrap{{position:relative;height:330px}}.cml-tabs-bar{{display:flex;gap:10px;margin:18px 0 14px;overflow-x:auto}}.cml-tab-btn{{border:1px solid #cfe2e7;border-radius:12px;padding:10px 16px;background:#fff;color:#3b6371;cursor:pointer;font-size:13px;font-weight:800;white-space:nowrap}}.cml-tab-btn.active{{border-color:#087087;background:linear-gradient(135deg,#0d90a7,#087087);color:#fff}}.cml-table-wrap{{width:100%;overflow-x:auto;border:1px solid #d5e4e9;border-radius:18px;background:#fff}}.cml-table{{width:100%;min-width:1080px;border-collapse:separate;border-spacing:0;font-size:13px;white-space:nowrap}}.cml-table th{{padding:15px 10px;background:linear-gradient(135deg,#0b7f98,#075d74);color:#fff;font-size:11px;font-weight:850;text-align:center}}.cml-table td{{padding:11px 10px;border-bottom:1px solid #e7eff1;color:#284d5a;text-align:right}}.cml-table td:first-child{{color:#087087;font-weight:850;text-align:center}}.cml-table td:nth-child(2){{text-align:left}}.cml-scenario-cell{{display:flex;align-items:center;gap:9px;min-width:180px;font-weight:650}}.cml-table-icon{{display:inline-flex;align-items:center;justify-content:center;flex:0 0 38px;width:38px;height:38px;overflow:visible;border:1px solid #e2edef;border-radius:10px;background:#fff;font-size:18px}}.cml-weather-svg{{display:block;width:35px;height:35px;overflow:visible}}.cml-night-row{{background:linear-gradient(90deg,#142549,#203b68)!important}}.cml-night-row td{{border-bottom-color:rgba(255,255,255,.10);color:#e5f0ff}}.cml-night-row td:first-child{{color:#a8eaf5}}.cml-night-row .cml-scenario-cell{{color:#ffedb2}}.cml-night-row .cml-table-icon{{border-color:rgba(255,255,255,.22);background:#142549;box-shadow:0 2px 8px rgba(253,230,138,.18)}}
...
try:
    ...
    with st.spinner(f"Elaborazione previsione ICON-2I e radar per {luogo}..."):
        ...
        is_costiero = any(normalizza_comune(candidato) in COMUNI_COSTIERI for candidato in candidati_localita)
        dati_mare = None
    ...
    documento = genera_app_completa(luogo, latitudine, longitudine, dati_terrestri, dati_orari, dati_giornalieri, dati_mare, distanza_mare_km)
